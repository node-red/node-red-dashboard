...
function MainController(t,e,n,i,a){function o(t,e){for(var n=0;n<e.length;n++){var i=e[n];if(i.id===t)return i;if(i.items){var a=o(t,i.items);if(a)return a}}}var l=this;this.tabs=[],this.selectedTab=null,this.loaded=!1,this.toggleSidenav=function(){t("left").toggle()},this.select=function(e){l.selectedTab=l.tabs[e],t("left").close(),i.path(e)},this.openEditor=function(){e.open("../","_blank"),t("left").close()},n.connect(function(t,e){l.tabs=t.tabs,a[0].title=t.title;var n=parseInt(i.path().substr(1));!isNaN(n)&&n<l.tabs.length?l.selectedTab=l.tabs[n]:l.select(0),e()},function(){l.loaded=!0}),n.on(function(t){var e=o(t.id,l.tabs);for(var n in t)"id"!==n&&(e[n]=t[n])})}var app=angular.module("ui",["ngMaterial","ngMdIcons","ngSanitize"]);app.config(["$mdThemingProvider",function(t){t.theme("default").primaryPalette("light-green").accentPalette("red")}]),app.controller("MainController",MainController),MainController.$inject=["$mdSidenav","$window","WebEvents","$location","$document"];
function WebEvents(n,t){var o="update-value";this.connect=function(e,i){var s=io({path:location.pathname+"socket.io"});this.emit=s.emit.bind(s,o),this.on=function(t){s.on(o,function(o){n(function(){t(o)},0)})},s.on("ui-controls",function(t){n(e(t,function(){s.emit("ui-replay-state")}),0)}),s.on("ui-replay-done",function(){n(i,0)}),s.on("show-toast",function(n){var o=t.simple().content(n.message).position("top right").hideDelay(3e3);t.show(o)})}}angular.module("ui").service("WebEvents",WebEvents),WebEvents.$inject=["$timeout","$mdToast"];
function UiComponent(t,e,n,i){var o={};return{restrict:"E",bindToController:{item:"="},controller:"uiComponentController",controllerAs:"me",scope:!0,link:function(r,l,m,a){var c=o[a.item.type];if(!c){var p="templates/controls/"+a.item.type+".html",u=n.get(p);c=u?i.when(u):t.get(p).then(function(t){return t.data}),o[a.item.type]=c}c.then(function(t){var n=angular.element(t);a.item.width&&n.attr("flex",a.item.width),l.replaceWith(e(n)(r))}),a.init()}}}angular.module("ui").directive("uiComponent",UiComponent),UiComponent.$inject=["$http","$compile","$templateCache","$q"];
function UiComponentController(t,i){this.init=function(){switch(this.item.type){case"numeric":this.item.getText=i(this.item.format||"{{value}}").bind(null,this.item);break;case"text":this.item.getText=i(this.item.format||"{{payload}}").bind(null,this.item)}},this.buttonClick=function(t){t&&(this.item.value=t),this.valueChanged(0)},this.valueChanged=function(t){n({id:this.item.id,value:this.item.value},"number"==typeof t?t:200)},this.valueDown=function(){this.item.value>this.item.min&&(this.item.value--,this.valueChanged(0))},this.valueUp=function(){this.item.value<this.item.max&&(this.item.value++,this.valueChanged(0))};var e,n=function(i,n){return 0===n?void t.emit(i):(e&&clearTimeout(e),void(e=setTimeout(function(){e=void 0,t.emit(i)},n)))}}angular.module("ui").controller("uiComponentController",UiComponentController),UiComponentController.$inject=["WebEvents","$interpolate"];
function UiCompile(i,e){return function(n,o,t){var c=e.$new();n.$watch("me.item.msg",function(i){c.msg=i}),n.$watch(function(i){return i.$eval(t.uiCompile)},function(e){o.html(e),i(o.contents())(c)})}}angular.module("ui").directive("uiCompile",UiCompile),UiCompile.$inject=["$compile","$rootScope"];
angular.module("ui").run(["$templateCache",function(e){e.put("templates/cards.html",'<md-toolbar class="md-whiteframe-z1"><div class="md-toolbar-tools"><md-button ng-click="main.toggleSidenav()" class="md-icon-button"><ng-md-icon icon="menu"></ng-md-icon></md-button><h1 ng-show="main.loaded">{{main.selectedTab.header}}</h1></div></md-toolbar><md-content layout="row" flex=""><md-sidenav layout="column" class="md-sidenav-left md-whiteframe-z1" md-component-id="left"><md-list><md-subheader class="md-no-sticky">Pages</md-subheader><md-list-item ng-repeat="tab in main.tabs" ng-click="main.select($index)"><ng-md-icon class="md-default-theme" icon="{{tab.icon}}" style="margin: 5px"></ng-md-icon><p>{{tab.header}}</p></md-list-item><md-subheader class="md-no-sticky">Other</md-subheader><md-list-item ng-click="main.openEditor()"><ng-md-icon icon="open_in_browser" style="margin: 5px"></ng-md-icon><p>Edit flows</p></md-list-item></md-list></md-sidenav><md-content layout="column" flex=""><md-content layout="row" layout-align="center start" layout-wrap=""><md-card ng-repeat="group in main.selectedTab.items" flex="" class="group-card"><md-card-content layout="column"><h2 ng-show="group.header" class="md-title">{{group.header}}</h2><div ng-repeat="control in group.items" flex=""><ui-component item="control"></ui-component><md-divider ng-if="!$last"></md-divider></div></md-card-content></md-card></md-content></md-content></md-content>'),e.put("templates/controls/button-row.html",'<div flex="" layout="row" layout-align="space-around center"><md-button ng-repeat="button in me.item.buttons" class="md-icon-button" ng-click="me.buttonClick(button.payload)" aria-label="unknown"><ng-md-icon icon="{{button.icon}}"></ng-md-icon></md-button></div>'),e.put("templates/controls/button.html",'<md-button class="md-raised" ng-click="me.buttonClick()" aria-label="{{me.item.label}}">{{me.item.label}}</md-button>'),e.put("templates/controls/color.html",'<div flex="" layout="row" layout-align="space-between center"><p>{{me.item.label}}</p><p>TODO...</p></div>'),e.put("templates/controls/numeric.html",'<div flex="" layout="row" layout-align="space-between center"><p>{{me.item.label}}</p><div layout="row" layout-align="center center"><md-button class="md-icon-button" ng-click="me.valueDown()" aria-label="{{me.item.label}}"><ng-md-icon icon="expand_more"></ng-md-icon></md-button><h3 ng-bind-html="me.item.getText()"></h3><md-button class="md-icon-button" ng-click="me.valueUp()" aria-label="{{me.item.label}}"><ng-md-icon icon="expand_less"></ng-md-icon></md-button></div></div>'),e.put("templates/controls/paragraph.html",'<div flex=""><p ui-compile="me.item.format"></p></div>'),e.put("templates/controls/slider.html",'<div flex="" layout="row" layout-align="space-between center"><p>{{me.item.label}}</p><md-slider flex="60" min="{{me.item.min}}" max="{{me.item.max}}" ng-model="me.item.value" aria-label="{{me.item.label}}" ng-change="me.valueChanged()"></md-slider></div>'),e.put("templates/controls/switch.html",'<div flex="" layout="row" layout-align="space-between center"><p>{{me.item.label}}</p><md-switch ng-model="me.item.value" ng-change="me.valueChanged(0)" aria-label="{{me.item.label}}"></md-switch></div>'),e.put("templates/controls/text.html",'<div flex="" layout="row" layout-align="space-between center"><p>{{me.item.label}}</p><h4 ng-bind-html="me.item.getText()"></h4></div>')}]);