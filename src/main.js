...
app.controller("MainController", [
  ...
  function (
      ...
      if (menu.link === undefined) {
        ...
        // open in iframe  (if allowed by remote site)
          ...
          if (typeof main.menu[index].link === "string") {
            main.menu[index].link = $sce.trustAsResourceUrl(
    ...
    function applyStyle(theme) {
      ...
      if (typeof main.allowTempTheme === "undefined") {
        main.allowTempTheme = true;
      }
      var queryString = window.location.search;
      var urlParams = new URLSearchParams(queryString);
      if (urlParams.get("font") === "lato") {
        lessObj["@baseFont"] =
          "-apple-system,Lato,Roboto,Oxygen-Sans,sans-serif";
      }

      if (urlParams.get("theme") === "angular") {
        lessObj["@nrTheme"] = false;
      } else {
      ...
      console.log(main, theme, lessObj);
      ...
      for (var t in main.menu) {
        if (main.menu.hasOwnProperty(t)) {
          if (typeof localStorage !== "undefined") {
            if (
              localStorage.getItem("th" + t + main.menu[t].header) == "true"
            ) {
              if (main.menu[t].hidden === true) {
                localStorage.removeItem("th" + t + main.menu[t].header);
              } else {
            ...
            if (
              localStorage.getItem("th" + t + main.menu[t].header) == "false"
            ) {
              if (main.menu[t].hidden === false) {
                localStorage.removeItem("th" + t + main.menu[t].header);
              } else {
            ...
            if (
              localStorage.getItem("td" + t + main.menu[t].header) == "true"
            ) {
              if (main.menu[t].disabled === true) {
                localStorage.removeItem("td" + t + main.menu[t].header);
              } else {
            ...
            if (
              localStorage.getItem("td" + t + main.menu[t].header) == "false"
            ) {
              if (main.menu[t].disabled === false) {
                localStorage.removeItem("td" + t + main.menu[t].header);
              } else {
          ...
          for (var g in main.menu[t].items) {
            if (main.menu[t].items.hasOwnProperty(g)) {
              var c = (
                main.menu[t].header +
                " " +
                main.menu[t].items[g].header.name
              ).replace(/ /g, "_");
              if (
                typeof localStorage !== "undefined" &&
                localStorage.getItem("g" + c) == "true"
              ) {
    ...
    events.connect(
      function (ui, done) {
        ...
        if (ui.site) {
          ...
          if (typeof ui.site.allowTempTheme === "undefined") {
            main.allowTempTheme = true;
          } else {
        ...
        $("meta[name=apple-mobile-web-app-title]").attr(
          "content",
          name || "Node-RED"
        );
        ...
        var finishLoading = function () {
          if (main.selectedTab && typeof main.selectedTab.theme === "object") {
            main.selectedTab.theme.themeState["widget-borderColor"] =
              main.selectedTab.theme.themeState["widget-borderColor"] ||
              main.selectedTab.theme.themeState["group-backgroundColor"];
            applyStyle(main.selectedTab.theme);
          } else if (
            typeof ui.theme === "object" &&
            ui.theme.themeState["base-color"].value
          ) {
            applyStyle(ui.theme);
          }
          if (
            main.selectedTab !== null &&
            main.selectedTab.link !== undefined
          ) {
            main.selectedTab.link = $sce.trustAsResourceUrl(
          ...
          if (ui.hasOwnProperty("theme")) {
            $("meta[name=theme-color]").attr(
              "content",
              ui.theme.themeState["page-titlebar-backgroundColor"].value ||
                "#097479"
            );
          } else {
    ...
    function arrayIncludesName(strArray, strName) {
      ...
      var arrNames =
        strArray && Array.isArray(strArray) ? strArray : [strArray];
    ...
    events.on(function (msg) {
      ...
      if (msg.hasOwnProperty("msg") && msg.msg.templateScope === "global") {
        ...
        if (
          found.hasOwnProperty("me") &&
          found.me.hasOwnProperty("processInput")
        ) {
    ...
    events.on("disconnect", function (m) {
      if (!disc && doVisualUpdates) {
        $mdToast.show({
          template:
            '<md-toast><div class="md-toast-error">&#x2718; &nbsp; Connection lost</div></md-toast>',
          position: "top right",
          hideDelay: 6000000,
        });
        disc = true;
    ...
    events.on("show-toast", function (msg) {
      ...
      if (msg.dialog === true) {
        ...
        if (msg.hasOwnProperty("message") || msg.hasOwnProperty("title")) {
          ...
          if (msg.hasOwnProperty("message") && msg.message == "") {
            msg.displayTime = 1;
          }
          var opts = {
            scope: toastScope,
    ...
    events.on("ui-control", function (msg) {
      if (msg.hasOwnProperty("socketid") && msg.socketid !== events.id) {
        return;
      }
      if (msg.hasOwnProperty("control")) {
        // if it's a request to modify a control
        var found = findControl(msg.id, main.menu);
        for (var property in msg.control) {
          if (
            msg.control.hasOwnProperty(property) &&
            found.hasOwnProperty(property)
          ) {
      ...
      if (msg.hasOwnProperty("tabs")) {
        // ui_control request to show/hide/enable/disable tabs
      ...
      if (msg.hasOwnProperty("tab")) {
        // if it's a request to change tabs
        ...
        if (index < main.menu.length) {
          if (!main.menu[index].disabled) {
            main.open(main.menu[index], index);
          }
          $scope.$apply();
          return;
        }
      }
      if (msg.hasOwnProperty("group")) {
        // it's to control a group item
    ...
    events.on("ui-audio", function (msg) {
      ...
      if (!msg.always) {
        var totab;
        for (var i in main.menu) {
          if (msg.tabname === main.menu[i].header) {
            totab = i;
          }
        }
        // only play sound/tts to tab if in focus
        if (totab != parseInt($location.path().substr(1))) {
          return;
        }
      }
      if (msg.hasOwnProperty("tts")) {
        if (voices.length > 0) {
          ...
          words.onerror = function (err) {
            events.emit("ui-audio", "error: " + err.error);
          };
