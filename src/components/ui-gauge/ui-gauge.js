            ...
            link: function(scope, element, attrs) {
                $timeout(function() {
                    ...
                    if (scope.$eval('me.item.gtype') === 'wave') {
                        scope.$watchGroup(['me.item.min','me.item.max','me.item.units','me.item.waveoptions','me.item.options'], function() {
                            if (unreg) { unreg(); }
                            //document.getElementById("gauge"+scope.$eval('$id')).innerHTML = "";
                    ...
                    // Justgage type gauges
                        ...
                        scope.$watchGroup(['me.item.min','me.item.max','me.item.seg1','me.item.seg2','me.item.colors','me.item.options'], function() {
                            if (unreg) { unreg(); }
