    ...
    function($timeout) {
        return {
            ...
            link: function (scope, element, attrs, controller) {
                scope.collapsed = ((((typeof sessionStorage !== 'undefined') && sessionStorage.getItem(attrs.id)) || false) == 'true');
                var root = element.find(".nr-dashboard-cardcontainer");
                var slideDuration = 0;
                controller.init(root);
                scope.collapseCard = function() {
                    scope.collapsed = !scope.collapsed;
                    if (typeof sessionStorage !== 'undefined') { sessionStorage.setItem(attrs.id,scope.collapsed); }
